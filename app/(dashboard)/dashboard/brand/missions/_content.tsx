...
import {
  ...
  DollarSign, Calendar, Tag, AlertCircle, Eye, RefreshCw,
} from "lucide-react";
...
export default function BrandMissionsContent() {
  ...
  const load = useCallback(async () => {
    ...
    try {
      ...
      const raw: Array<{
        id: string; title: string; brief: string; budget: number; type: string; status: string;
        contentUrl: string | null; deliveryDate: string | null; createdAt: string;
  ...
  async function handleValidate(id: string) {
    ...
    try {
      const res = await fetch(`/api/missions/${id}/pay`, { method: "POST" });
      if (!res.ok) {
        const d = await res.json();
        toast.error(d.error ?? "Erreur");
        return;
      }
      setMissions((ms) => ms.map((m) => m.id === id ? { ...m, status: "PAID" } : m));
  ...
  async function handleReject(id: string) {
    setActionLoading((l) => ({ ...l, [`${id}_reject`]: true }));
    try {
      const res = await fetch(`/api/missions/${id}`, {
        method:  "PATCH",
        headers: { "Content-Type": "application/json" },
        body:    JSON.stringify({ status: "PENDING" }), // reset to pending for revision
      });
      if (!res.ok) {
        const d = await res.json();
        toast.error(d.error ?? "Erreur");
      ...
      setActionLoading((l) => ({ ...l, [`${id}_reject`]: false }));
    }
  }
  ...
  return (
    <div className="max-w-5xl space-y-6">
        ...
        action={
          <div className="flex items-center gap-2">
            ...
            <Link href="/dashboard/brand/missions/new">
              <Button variant="primary"><PlusCircle size={15} /> Nouvelle mission</Button>
            </Link>
          </div>
        }
      />
      ...
      {!loading && toValidateCount > 0 && (
        <div className="flex items-center gap-3 p-4 bg-emerald-500/[0.08] border border-emerald-500/20 rounded-[14px]">
          <Package size={16} className="text-emerald-400 flex-shrink-0" />
          <p className="text-sm text-slate-300 flex-1">
            <span className="font-semibold text-emerald-400">{toValidateCount} livraison{toValidateCount > 1 ? "s" : ""}</span> en attente de validation.
          </p>
                  ...
                  {isOpen && (
                    <div className="px-6 pb-6 space-y-4">
                        ...
                        <div className="space-y-3">
                          <div className="flex items-start gap-2.5 p-3 bg-emerald-500/5 border border-emerald-500/20 rounded-[10px]">
                            ...
                            <p className="text-xs text-slate-500">
                              Vérifie que le contenu est conforme au brief. Le paiement de <span className="text-emerald-400 font-semibold">{formatMAD(m.budget * 0.9)}</span> sera libéré immédiatement.
                            </p>
                          </div>
                          <div className="flex gap-3">
                            <Button variant="secondary" size="sm" className="flex-1"
                              loading={actionLoading[`${m.id}_reject`]}
                      ...
                      <div className="flex justify-end">
                        <Link href={`/creator/${m.creatorId}`} target="_blank">
